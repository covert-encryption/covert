      ...
      if len(words) > 0:
        for word in words:
          ...
          while len(word[maxi:]) >= MAX_CHARACTERS:
            if curlength < 1:
              wrapped[len(wrapped) - 1] += (word[maxi:maxi - 1 + MAX_CHARACTERS])
              curlength = len(word[maxi:maxi + MAX_CHARACTERS])
            else:
              wrapped.append(word[maxi:maxi - 1 + MAX_CHARACTERS])
              curlength = len(wrapped[len(wrapped) - 1])
              linenums.append(i)
...
def edit(term, lines: List[str]):
  ...
  while True:
    ...
    wrapped, linenums = word_wrap(lines)

    # gets the right displayed row, accounting for above wrapped lines
    drow = 0
    if row > 0:
      drow -= 2
      for id, line in enumerate(wrapped):
        if linenums[id] <= row:
          if linenums[id] == row:
    ...
    if dcol == 0:
      if len(wrapped[drow]) > 0 and wrapped[drow][0].isspace():
        drow -= 1
        dcol = len(wrapped[drow])

    startrow = min(max(0, drow - 1), startrow)
    startrow = max(drow - win.lines + 2, startrow)

    draw = "\x1B[0K\n".join(l[:win.columns - 1] for l in wrapped[startrow:startrow + win.lines - 1])
    ...
    for key in term.reader():
      ...
      # move to lower line)
        ...
        if drow > 0 and len(lines[row]) > MAX_CHARACTERS:
          ...
          if ((dcol - col == 0) or (dcol - col == 1)) and row > 0:
            ...
            if len(lines[row]) > MAX_CHARACTERS:
              ...
              col = (len(lines[row]) - len(wrapped[drow - 1])) + min(col, len(wrapped[drow - 1]))
            else:
              col = min(dcol, len(lines[row]))
          else:
            # if moving to a shorter line than current
            if len(wrapped[drow - 1]) < dcol:
              col -= dcol - len(wrapped[drow - 1])

            col -= dcol + (MAX_CHARACTERS - dcol) - 2
            # accounts for empty area in a word wrapped line
            col += MAX_CHARACTERS - len(wrapped[drow - 1]) - 2
        ...
        if row < len(lines) and drow < len(wrapped) - 1 and len(lines[row]) > MAX_CHARACTERS:

          # end of wrapped line reached, go to next line
          eol = col + len(wrapped[drow]) - dcol
          if len(lines[row]) == eol:

            row += 1
            if len(lines) == row:
              lines.insert(row, "")
            col = min(len(wrapped[drow + 1]), dcol)
          else:
            ...
            if len(wrapped[drow]) == MAX_CHARACTERS - 1:
              col += 1

            # accounts for empty area in a word wrapped line
            col -= max(0, MAX_CHARACTERS - len(wrapped[drow]) - 2)
        elif row < len(lines) - 1:
          row += 1
          col = min(col, len(lines[row]))
      elif key == 'ENTER':
